...
async function writeMetric({ root, name, frontmatter, body }) {
...
async function createRepo() {
  ...
  await fs.mkdir(metricsDir, { recursive: true });
  await fs.writeFile(
    path.join(metricsDir, "phases.json"),
...
describe("buildMetricsIndex", () => {
  it("builds an index from valid metrics", async () => {
  ...
  it("fails when the metric phase is unknown", async () => {
    const root = await createRepo();
  ...
  it("fails on duplicate metric ids", async () => {
    const root = await createRepo();
  ...
  it("fails when depends_on references unknown metrics", async () => {
    const root = await createRepo();
  ...
  it("fails when the metric body is empty", async () => {
    const root = await createRepo();
